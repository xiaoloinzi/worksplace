...
import time
from AppTestGR.lib.appopera import find_element
...
def app_is_exist(driver,**kwargs):
    #循环校验，没两秒一次，存在则退出，循环结束，还不存在，则断言失败.(10)
    assert 'app_is_exist.appPkgName' in kwargs.keys()
    for i in xrange(10):
        time.sleep(2)
        isInstalled = driver.is_app_installed(kwargs['app_is_exist.appPkgName'])
        if isInstalled is True:
            break
        if i == 9:
            assert False

def download_app_notexist(driver,**kwargs):
    #下载不存的app
    assert 'download_app_notexist.appName' in kwargs.keys()
    time.sleep(5)
    el1 = find_element(driver,'com.huawei.appmarket:id/search_edit_text')
    el1.click()

    time.sleep(1)

    el2 = find_element(driver,'com.huawei.appmarket:id/searchText')
    el2.clear()
    # el2.set_value(r'#()*')
    el2.set_text(unicode(kwargs['download_app_notexist.appName']))
    time.sleep(2)
    el3 = find_element(driver,'com.huawei.appmarket:id/search_title_icon')
    el3.click()
    time.sleep(1)

    el4 = find_element(driver,u'//android.widget.TextView[@text="没有符合条件的内容"]')
...
def open_switch(driver,**kwargs):
    ...
    print el3.get_attribute('checked')
